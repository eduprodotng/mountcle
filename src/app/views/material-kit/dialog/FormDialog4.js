...
export default function FormDialog4({ updateTableData }) {
  ...
  console.log("this is the current ession", currentSession);
  const {
    data: teachersData,
    loading: teachersLoading,
    error: teachersError,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ensure the response has data before proceeding
      if (response.status === 201 || response.status === 200) {
        toast.success("Subject saved successfully!");

        try {
          updateTableData(response.data); // Update table
          handleClose(); // Close modal
        } catch (innerError) {
          console.error("Error updating table data:", innerError);
        }
      } else {
